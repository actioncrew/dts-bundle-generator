...
export function generateDtsBundle(entries: readonly EntryPointConfig[], options: CompilationOptions = {}): string[] {
  ...
  return entries.map((entryConfig: EntryPointConfig) => {
    ...
    function isIdentifier(node: ts.ModuleExportName): node is ts.Identifier {
      return ts.isIdentifier(node);
    }

    function getIdentifierFromModuleExportName(name: ts.ModuleExportName): ts.Identifier | null {
    ...
    function addNamedImport(importItem: ModuleImportsSet, preferredLocalName: ts.ModuleExportName, importedIdentifier: ts.ModuleExportName): void {
      const preferredId = getIdentifierFromModuleExportName(preferredLocalName);
      const importedId = getIdentifierFromModuleExportName(importedIdentifier);
      
      if (!preferredId || !importedId) {
        // Handle string literal case - you might want to create a synthetic identifier
        // or throw an error depending on your requirements
        return;
      }
      
      const newLocalName = collisionsResolver.addTopLevelIdentifier(preferredId);
      const importedName = importedId.text;
    ...
    function addNsImport(importItem: ModuleImportsSet, preferredLocalName: ts.ModuleExportName): void {
      ...
      if (importItem.nsImport === null) {
        importItem.nsImport = collisionsResolver.addTopLevelIdentifier(preferredId);
      }
    }
      ...
      function getIdentifierOfNamespaceImportFromInlinedModule(nsSymbol: ts.Symbol): ts.Identifier | null {
        ...
        for (const decl of getDeclarationsForSymbol(nsSymbol)) {
          ...
          if (ts.isNamespaceExport(decl) && !isReferencedModuleImportable(decl.parent)) {
            return getIdentifierFromModuleExportName(decl.name);
          }
          ...
          case ModuleType.ShouldBeInlined: {
            ...
            forEachImportOfStatement(sourceFile, (imp: ImportOfStatement) => {
              ...
              if (ts.isNamespaceExport(imp) || ts.isNamespaceImport(imp)) {
                namespaceIdentifier = getIdentifierFromModuleExportName(imp.name);
              }
            });
